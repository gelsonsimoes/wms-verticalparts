...
export default function IntegrationWaves() {
  ...
  const handleSaveWaves = () => {
    ...
    alert('Configurações de Onda salvas com sucesso!');
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 p-8 pt-10 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tight text-white uppercase">Parâmetros de Sistema</h1>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-900 border-b border-slate-800">
                ...
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500">Formato</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500">Periodicidade</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500">Hora</th>
                ...
                <tr key={item.id} className="hover:bg-slate-800/30 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                       <div className={cn("w-2 h-2 rounded-full shadow-[0_0_8px]", item.active ? "bg-green-500 shadow-green-500/50" : "bg-slate-600 shadow-slate-600/50")} />
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="p-2 hover:bg-white/5 rounded-lg text-slate-400 transition-all"><Plus className="w-4 h-4" /></button>
                      <button className="p-2 hover:bg-white/5 rounded-lg text-slate-400 transition-all"><Trash2 className="w-4 h-4 hover:text-red-500" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <div className="space-y-6">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl relative overflow-hidden">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Nomenclatura do Arquivo</label>
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Formatação</label>
                <select className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-primary transition-all text-slate-200 appearance-none cursor-pointer">
            ...
            <div className="bg-slate-950/50 border border-slate-800/60 rounded-xl p-4 space-y-4 mt-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-bold text-slate-600 uppercase ml-1">Periodicidade</label>
                    <select className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-primary transition-all text-slate-400 appearance-none cursor-pointer">
                      {PERIOD_OPTIONS.map(opt => <option key={opt}>{opt}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-bold text-slate-600 uppercase ml-1">Hora Execução</label>
                    <input 
                      type="time" 
            ...
            <button className="w-full mt-4 bg-primary text-slate-950 font-black py-4 rounded-xl flex items-center justify-center gap-2 hover:brightness-110 active:scale-[0.98] transition-all shadow-lg shadow-primary/20 text-xs uppercase tracking-widest">
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-gradient-to-br from-primary/10 via-slate-900 to-slate-950 border border-primary/20 rounded-3xl p-8 relative overflow-hidden group">
        <div className="absolute -right-4 -top-4 w-48 h-48 bg-primary/5 rounded-full blur-3xl transition-all group-hover:bg-primary/10" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 space-y-6 shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider ml-1">Peso Mínimo (kg)</label>
              <div className="relative">
                <Database className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-700" />
                <input 
                  type="number" 
                  value={config.minWeight}
                  onChange={(e) => updateConfig('minWeight', e.target.value)}
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider ml-1">Peso Máximo (kg)</label>
              <div className="relative">
                <Database className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-700" />
                <input 
                  type="number" 
                  value={config.maxWeight}
                  onChange={(e) => updateConfig('maxWeight', e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-5 py-4 text-lg font-black text-primary focus:outline-none focus:border-primary/50 transition-all font-mono" 
                />
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-6 pt-2">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider ml-1">Número Máx Peças</label>
              <div className="relative">
                <input 
                  type="number" 
                  value={config.maxItems}
                  onChange={(e) => updateConfig('maxItems', e.target.value)}
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider ml-1">Máximo SKUs</label>
              <div className="relative">
                <input 
                  type="number" 
                  value={config.maxSkus}
                  onChange={(e) => updateConfig('maxSkus', e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-5 py-4 text-lg font-black text-white focus:outline-none focus:border-primary/50 transition-all font-mono" 
                />
              </div>
            </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 flex flex-col shadow-2xl border-l-4 border-l-primary/30">
          ...
          <div className="space-y-6 flex-1">
            <label className="flex items-center justify-between p-4 bg-slate-950 border border-slate-800 rounded-2xl hover:border-primary/30 transition-all cursor-pointer group">
              <div className="flex items-center gap-4">
                <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center transition-all", config.groupNf ? "bg-primary/20 text-primary" : "bg-slate-800 text-slate-600")}>
                  <FileText className="w-5 h-5" />
                </div>
                <div>
                  <p className="text-xs font-black text-white uppercase tracking-tight">Agrupar NF na Formação</p>
                  <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Prioriza remessa por documento</p>
                </div>
              </div>
              <div 
                onClick={() => updateConfig('groupNf', !config.groupNf)}
                className={cn("w-12 h-6 rounded-full relative transition-all", config.groupNf ? "bg-primary" : "bg-slate-700")}
              >
                <div className={cn("absolute top-1 w-4 h-4 rounded-full bg-white transition-all shadow-lg", config.groupNf ? "left-7" : "left-1")} />
              </div>
            </label>

            <label className="flex items-center justify-between p-4 bg-slate-950 border border-slate-800 rounded-2xl hover:border-primary/30 transition-all cursor-pointer group">
              <div className="flex items-center gap-4">
                <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center transition-all", config.allowBreak ? "bg-primary/20 text-primary" : "bg-slate-800 text-slate-600")}>
                  <Workflow className="w-5 h-5" />
                </div>
                <div>
                  <p className="text-xs font-black text-white uppercase tracking-tight">Permitir Quebra de Local</p>
                  <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Divide ordem em múltiplos setores</p>
                </div>
              </div>
              <div 
                onClick={() => updateConfig('allowBreak', !config.allowBreak)}
                className={cn("w-12 h-6 rounded-full relative transition-all", config.allowBreak ? "bg-primary" : "bg-slate-700")}
              >
                <div className={cn("absolute top-1 w-4 h-4 rounded-full bg-white transition-all shadow-lg", config.allowBreak ? "left-7" : "left-1")} />
              </div>
            </label>

            <label className="flex items-center justify-between p-4 bg-slate-950 border border-slate-800 rounded-2xl hover:border-primary/30 transition-all cursor-pointer group">
              <div className="flex items-center gap-4">
                <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center transition-all", config.useOutputCheck ? "bg-primary/20 text-primary" : "bg-slate-800 text-slate-600")}>
                  <CheckCircle2 className="w-5 h-5" />
                </div>
                <div>
                  <p className="text-xs font-black text-white uppercase tracking-tight">Conferência de Saída</p>
                  <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Exige validação final (Checking)</p>
                </div>
              </div>
              <div 
                onClick={() => updateConfig('useOutputCheck', !config.useOutputCheck)}
                className={cn("w-12 h-6 rounded-full relative transition-all", config.useOutputCheck ? "bg-primary" : "bg-slate-700")}
              >
                <div className={cn("absolute top-1 w-4 h-4 rounded-full bg-white transition-all shadow-lg", config.useOutputCheck ? "left-7" : "left-1")} />
              </div>
            </label>
          </div>
          ...
          >
            <Save className="w-5 h-5" /> Aplicar Regras de Onda
          </button>
        </div>
      </div>
