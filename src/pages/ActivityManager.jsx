...
export default function ActivityManager() {
  ...
  // ── Central de Aprovação (registros_campo_pendentes) ─────────────
  const [registrosPendentes, setRegistrosPendentes] = useState([]);
  const [loadingRegistros, setLoadingRegistros]     = useState(true);
  const [aprovandoId, setAprovandoId]               = useState(null);

  const fetchRegistrosPendentes = useCallback(async () => {
    const { data } = await supabase
      .from('registros_campo_pendentes')
      .select('*')
      .order('created_at', { ascending: false })
      .limit(50);
    setRegistrosPendentes(data || []);
    setLoadingRegistros(false);
  }, []);

  useEffect(() => { fetchRegistrosPendentes(); }, [fetchRegistrosPendentes]);

  useEffect(() => {
    const ch = supabase
      .channel('campo-pendentes-feed')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'registros_campo_pendentes' },
        (payload) => setRegistrosPendentes(prev => [payload.new, ...prev])
      )
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'registros_campo_pendentes' },
        () => fetchRegistrosPendentes()
      )
      .subscribe();
    return () => { supabase.removeChannel(ch); };
  }, [fetchRegistrosPendentes]);

  const handleAprovar = async (id, tipo, referenciaId) => {
    setAprovandoId(id);
    try {
      await supabase.from('registros_campo_pendentes').update({ status: 'aprovado', aprovado_por: 'gestor-web', aprovado_em: new Date().toISOString() }).eq('id', id);
      // Ativa o registro na tabela correspondente
      if (tipo === 'motorista') await supabase.from('motoristas').update({ status: 'ativo', aprovado_por: 'gestor-web', aprovado_em: new Date().toISOString() }).eq('id', referenciaId);
      if (tipo === 'veiculo')   await supabase.from('veiculos').update({ status: 'ativo', aprovado_por: 'gestor-web' }).eq('id', referenciaId);
      if (tipo === 'produto')   await supabase.from('produtos').update({ status: 'ativo' }).eq('id', referenciaId);
      setRegistrosPendentes(prev => prev.map(r => r.id === id ? { ...r, status: 'aprovado' } : r));
    } catch (e) { console.error('[handleAprovar]', e); }
    finally { setAprovandoId(null); }
  };

  const handleRejeitar = async (id, referenciaId, tipo) => {
    setAprovandoId(id);
    try {
      await supabase.from('registros_campo_pendentes').update({ status: 'rejeitado' }).eq('id', id);
      if (tipo === 'motorista') await supabase.from('motoristas').update({ status: 'inativo' }).eq('id', referenciaId);
      if (tipo === 'veiculo')   await supabase.from('veiculos').update({ status: 'inativo' }).eq('id', referenciaId);
      setRegistrosPendentes(prev => prev.map(r => r.id === id ? { ...r, status: 'rejeitado' } : r));
    } catch (e) { console.error('[handleRejeitar]', e); }
    finally { setAprovandoId(null); }
  };

  const pendentesCount = registrosPendentes.filter(r => r.status === 'pendente').length;
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[28px] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black uppercase tracking-widest text-slate-900 dark:text-white flex items-center gap-2">
                Novos Cadastros Mobile
                {pendentesCount > 0 && (
                  <span className="px-2 py-0.5 bg-red-500 text-white rounded-full text-[9px] font-black animate-pulse">{pendentesCount}</span>
                )}
              </h3>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Cadastros feitos em campo aguardando validação do gestor</p>
            </div>
          </div>
          <button onClick={fetchRegistrosPendentes} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 transition-colors" title="Atualizar">
        ...
        {loadingRegistros ? (
          ...
          <div className="flex flex-col items-center justify-center py-14 gap-3 text-slate-400">
            ...
            <p className="text-xs font-bold uppercase tracking-widest">Nenhum cadastro mobile pendente.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 p-6">
            {registrosPendentes.map((reg) => {
              const isPendente = reg.status === 'pendente';
              const isAprovado = reg.status === 'aprovado';
              const tipoIcon = reg.tipo === 'motorista' ? <User className="w-4 h-4" /> : reg.tipo === 'veiculo' ? <Truck className="w-4 h-4" /> : <Package className="w-4 h-4" />;
              const tipoLabel = reg.tipo === 'motorista' ? 'Motorista' : reg.tipo === 'veiculo' ? 'Veículo' : 'Produto';
              const dados = reg.dados || {};
              const isProcessing = aprovandoId === reg.id;
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isPendente ? 'border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/10' :
                    isAprovado ? 'border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/10 opacity-70' :
                ...
                >
                  ...
                  {reg.foto_url ? (
                    <div className="h-40 overflow-hidden relative">
                      <img src={reg.foto_url} alt="Foto de campo" className="w-full h-full object-cover" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />
                      <div className="absolute bottom-2 left-3 flex items-center gap-1.5 text-white text-[9px] font-black uppercase tracking-widest">
                        <Camera className="w-3 h-3" /> Foto de campo
                      </div>
                    </div>
                  ) : (
                    <div className="h-24 flex items-center justify-center bg-slate-100 dark:bg-slate-800">
                      <div className="text-slate-400 opacity-40">{tipoIcon}</div>
                    </div>
                  )}
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-2 mb-3">
                      <span className={cn('flex items-center gap-1.5 px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-widest',
                        reg.tipo === 'motorista' ? 'bg-blue-100 text-blue-700' :
                        reg.tipo === 'veiculo'   ? 'bg-orange-100 text-orange-700' : 'bg-purple-100 text-purple-700'
                      )}>{tipoIcon}{tipoLabel}</span>
                      <span className={cn('ml-auto text-[9px] font-black uppercase px-2 py-1 rounded-full',
                        isPendente ? 'bg-amber-200 text-amber-800' :
                        isAprovado ? 'bg-emerald-200 text-emerald-800' : 'bg-red-200 text-red-800'
                      )}>{reg.status}</span>
                    </div>
                    ...
                    <div className="space-y-1 mb-3">
                      {Object.entries(dados).map(([k, v]) => (
                        <div key={k} className="flex justify-between text-xs">
                          <span className="text-slate-400 font-bold uppercase tracking-wider">{k}</span>
                          <span className="font-black text-slate-800 dark:text-slate-200 max-w-[60%] text-right truncate">{String(v)}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center gap-1 text-[9px] text-slate-400 font-bold mb-3">
                      ...
                      {reg.operador_nome} · {reg.created_at ? new Date(reg.created_at).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : '—'}
                    </div>

                    {isPendente && (
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleAprovar(reg.id, reg.tipo, reg.referencia_id)}
                          disabled={isProcessing}
                          className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all disabled:opacity-50"
                        >
                          {isProcessing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <ThumbsUp className="w-3.5 h-3.5" />}
                          Aprovar
                        </button>
                        <button
                          onClick={() => handleRejeitar(reg.id, reg.referencia_id, reg.tipo)}
                          disabled={isProcessing}
                          className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-red-600 hover:bg-red-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all disabled:opacity-50"
                        >
