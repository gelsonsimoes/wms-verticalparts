...
function SupervisorModal({ onConfirm, onCancel, message }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={onCancel}>
            <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-sm p-8 space-y-6" onClick={e => e.stopPropagation()}>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-base font-black">Autorização de Supervisor</h3>
                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Ação restrita</p>
                    </div>
                </div>

                {message && <div className="flex items-start gap-2 p-3 bg-danger/5 border border-danger/20 rounded-xl"><AlertTriangle className="w-4 h-4 text-danger mt-0.5 shrink-0" /><p className="text-xs text-danger font-bold">{message}</p></div>}

                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="space-y-1.5">
                        <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Usuário Supervisor</label>
                        <input required type="text" value={user} onChange={e => setUser(e.target.value)} placeholder="Login do supervisor" className="w-full bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-xl py-3 px-4 text-sm font-bold focus:border-danger focus:ring-4 focus:ring-danger/10 outline-none transition-all" />
                    </div>
                    <div className="space-y-1.5">
                        <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Senha</label>
                        <input required type="password" value={pass} onChange={e => setPass(e.target.value)} placeholder="••••••••" className="w-full bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-xl py-3 px-4 text-sm font-bold focus:border-danger focus:ring-4 focus:ring-danger/10 outline-none transition-all" />
                    </div>
                    <div className="grid grid-cols-2 gap-3 pt-2">
                        ...
                        <button type="submit" className="py-3 bg-danger text-white font-black rounded-xl text-[10px] tracking-widest uppercase hover:bg-danger/90 transition-all shadow-lg shadow-danger/20 flex items-center justify-center gap-2"><Shield className="w-3.5 h-3.5" /> Autorizar</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    return (
        <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black tracking-tight">Gestão de Setores</h1>
                ...
                <div className="lg:col-span-4">
                    <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
                        ...
                        <form onSubmit={handleSave} className="space-y-4">
                            <div className="space-y-1.5">
                                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Id Setor</label>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1.5">
                                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Tipo de Setor</label>
                                    <select value={formData.tipoSetor} onChange={e => setFormData({ ...formData, tipoSetor: e.target.value })} className="w-full bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-xl py-2.5 px-4 text-xs font-bold focus:border-primary outline-none transition-all">
                                        {tipoSetorOptions.map(o => <option key={o}>{o}</option>)}
                                    </select>
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Tipo do Local</label>
                                    <select value={formData.tipoLocal} onChange={e => setFormData({ ...formData, tipoLocal: e.target.value })} className="w-full bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-xl py-2.5 px-4 text-xs font-bold focus:border-primary outline-none transition-all">
                                        {tipoLocalOptions.map(o => <option key={o}>{o}</option>)}
                                    </select>
                                </div>
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Código de Integração</label>
                            ...
                            <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800">
                                <button type="button" onClick={() => setFormData({ ...formData, usoExclusivoCaixa: !formData.usoExclusivoCaixa })}>
                                    {formData.usoExclusivoCaixa ? <ToggleRight className="w-7 h-7 text-warning" /> : <ToggleLeft className="w-7 h-7 text-slate-400" />}
                                </button>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm flex flex-col">
                        ...
                        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-wrap items-center gap-3 bg-slate-50/50 dark:bg-slate-900/50">
                            <div className="relative flex-1 min-w-[200px] max-w-xs">
                                <input type="text" placeholder="Procurar setor..." value={searchTerm} onChange={e => { setSearchTerm(e.target.value); setCurrentPage(1); }} className="w-full pl-9 pr-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold outline-none focus:border-primary transition-all" />
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="text-[8px] uppercase font-black tracking-[0.15em] text-slate-400 border-b border-slate-100 dark:border-slate-800">
                                        ...
                                        <th className="px-4 py-4">Tipo Setor</th>
                                        <th className="px-4 py-4">Tipo Local</th>
                                        <th className="px-4 py-4">Cód. Integração</th>
