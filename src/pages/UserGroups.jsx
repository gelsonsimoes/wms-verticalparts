...
function MultiSelectModal({ title, icon: Icon, allOptions, selected, onSave, onClose }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[90] flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-lg max-h-[75vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-1 overflow-auto p-4 space-y-1.5">
                    {allOptions.map(opt => (
                        <button key={opt} onClick={() => toggle(opt)} className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-all ${current.includes(opt) ? 'bg-primary/10 border-2 border-primary' : 'bg-slate-50 dark:bg-slate-900 border-2 border-transparent hover:border-slate-200'}`}>
                            {current.includes(opt) ? <CheckSquare className="w-4 h-4 text-primary shrink-0" /> : <Square className="w-4 h-4 text-slate-300 shrink-0" />}
                ...
                <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 flex gap-3">
                    ...
                    <button onClick={() => onSave(current)} className="flex-1 py-3 bg-secondary text-primary font-black rounded-xl text-[10px] tracking-widest uppercase hover:bg-secondary/90 transition-all shadow-lg shadow-black/10 flex items-center justify-center gap-2"><Save className="w-3.5 h-3.5" /> Salvar</button>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black tracking-tight">Grupo de Usuário</h1>
                ...
                <div className="lg:col-span-4">
                    <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
                        <div className="flex items-center gap-3 mb-6">
                            <div className="w-9 h-9 bg-secondary rounded-xl flex items-center justify-center"><Shield className="w-4 h-4 text-primary" /></div>
                            <h2 className="text-base font-black italic">{isEditing ? 'Alterar Grupo' : 'Novo Grupo'}</h2>
                        </div>

                        <form onSubmit={handleSave} className="space-y-4">
                            <div className="space-y-1.5">
                                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Id Grupo</label>
                            ...
                            <div className="space-y-1.5">
                                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Grupo</label>
                            ...
                            <div className="space-y-2">
                                <button type="button" onClick={() => setFormData({ ...formData, ativaExportacoes: !formData.ativaExportacoes })} className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-all border-2 ${formData.ativaExportacoes ? 'bg-primary/10 border-primary' : 'bg-slate-50 dark:bg-slate-900 border-transparent'}`}>
                                    {formData.ativaExportacoes ? <CheckSquare className="w-4 h-4 text-primary shrink-0" /> : <Square className="w-4 h-4 text-slate-300 shrink-0" />}
                                    <span className="text-[9px] font-black uppercase tracking-widest">Ativa exportações através das abas</span>
                                </button>
                                <button type="button" onClick={() => setFormData({ ...formData, permitirDownload: !formData.permitirDownload })} className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-all border-2 ${formData.permitirDownload ? 'bg-primary/10 border-primary' : 'bg-slate-50 dark:bg-slate-900 border-transparent'}`}>
                                    {formData.permitirDownload ? <CheckSquare className="w-4 h-4 text-primary shrink-0" /> : <Square className="w-4 h-4 text-slate-300 shrink-0" />}
                            ...
                            <div className="space-y-2 pt-2">
                                {isEditing ? (
                                    <>
                                        ...
                                        <button type="button" onClick={handleDelete} className="w-full py-2.5 bg-danger/10 text-danger font-black rounded-xl text-[10px] tracking-widest uppercase hover:bg-danger hover:text-white transition-all border border-danger/20 flex items-center justify-center gap-1.5"><Trash2 className="w-3.5 h-3.5" /> Excluir</button>
                                    </>
                                ) : (
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm flex flex-col">
                        <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                            <div className="relative max-w-xs">
                                <input type="text" placeholder="Procurar grupo..." value={searchTerm} onChange={e => { setSearchTerm(e.target.value); setCurrentPage(1); }} className="w-full pl-9 pr-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold outline-none focus:border-primary transition-all" />
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="text-[8px] uppercase font-black tracking-[0.15em] text-slate-400 border-b border-slate-100 dark:border-slate-800">
                                        ...
                                        <th className="px-4 py-4 text-center">Exportações</th>
                                ...
                                <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                                    {paginated.length > 0 ? paginated.map(g => (
                                        <tr key={g.id} onClick={() => handleSelect(g)} className={`cursor-pointer transition-all duration-200 ${selectedId === g.id ? 'bg-primary/10 border-l-4 border-l-primary' : 'hover:bg-slate-50 dark:hover:bg-slate-900/50'}`}>
                                            ...
                                            <td className="px-4 py-3.5 text-center">{g.ativaExportacoes ? <CheckSquare className="w-4 h-4 text-success mx-auto" /> : <Square className="w-4 h-4 text-slate-300 mx-auto" />}</td>
                                            <td className="px-4 py-3.5 text-center">{g.permitirDownload ? <CheckSquare className="w-4 h-4 text-success mx-auto" /> : <Square className="w-4 h-4 text-slate-300 mx-auto" />}</td>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                        {permissionButtons.map(perm => (
                            <button key={perm.key} onClick={() => setActiveModal(perm.key)} className="flex flex-col items-center gap-3 p-4 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-2xl hover:border-primary hover:bg-primary/5 transition-all group">
