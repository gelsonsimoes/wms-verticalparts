  ...
  return (
    <div role="alert" onClick={onClose}
      className={cn(
        ...
        type === 'erro'
          ? 'bg-red-50 text-red-700 border-red-200 dark:bg-red-950 dark:border-red-800 dark:text-red-400'
...
export default function WeighingStation() {
  const { warehouseId, serialDevices } = useApp();
  ...
  useEffect(() => {
    if (scales.length > 0 && !activeScale) setActiveScale(scales[0].nome);
  }, [scales, activeScale]);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (searchSku.length >= 2) fetchProducts(searchSku);
      else setProducts([]);
    }, 300);
  ...
  // ── Capturar peso (simula leitura serial) ─────────────────────────
  const handleCapture = () => {
    if (!selectedProduct) { showToast('Selecione um produto primeiro.', 'erro'); return; }
    setIsCapturing(true);
    setTimeout(() => {
  ...
  const handleSave = async () => {
    ...
    if (!isValidated) {
      showToast('Capture um peso válido antes de salvar.', 'erro');
      return;
    }
    setIsSaving(true);
    const { error } = await pesagensService.insert({
      ...
      peso_capturado: currentWeight,
      peso_master:    theoreticalWeight,
      variacao:       parseFloat((currentWeight - theoreticalWeight).toFixed(3)),
      modo,
  ...
  const actionGroups = [[
    { label: mode === 'INBOUND' ? 'Modo: Recebimento' : 'Modo: Expedição', icon: ArrowRightLeft, onClick: () => setMode(m => m === 'INBOUND' ? 'OUTBOUND' : 'INBOUND') },
    { label: 'Capturar Peso', icon: ArrowDownCircle, onClick: handleCapture, disabled: isCapturing || !selectedProduct },
    { label: 'Zerar Balança', icon: Trash2, onClick: handleZero },
    { label: isSaving ? 'Salvando...' : 'Salvar Pesagem', icon: Save, primary: true, onClick: handleSave, disabled: !isValidated || isBlocked || isSaving },
  ]];

  return (
    <EnterprisePageBase
      title="2.3 Estação de Pesagem"
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {selectedProduct && (
              <div className="mt-8 p-6 bg-primary/5 rounded-[32px] border-2 border-primary/20 flex flex-col md:flex-row items-center justify-between gap-6 animate-in zoom-in-95">
                ...
                <div className="flex gap-4">
                  ...
                  <div className="text-center px-6 py-3 bg-white dark:bg-slate-800 rounded-2xl border border-primary/10">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Tolerância</p>
        ...
        <div className="flex flex-col gap-4">
          <div className={cn(
            ...
            {!selectedProduct ? (
              <><Search className="w-16 h-16 text-slate-300 mb-4" /><p className="text-xs font-black uppercase tracking-widest text-slate-400">Aguardando Produto</p></>
            ) : isBlocked ? (
              <>
                ...
                <div className="mt-6 px-4 py-2 bg-red-600 text-white rounded-full text-[9px] font-black uppercase tracking-tighter">
                  Erro: {Math.abs(currentWeight - theoreticalWeight).toFixed(3)} KG
                </div>
              </>
            ) : isValidated ? (
              <>
          ...
          <div className="relative bg-slate-950 border-4 border-slate-800 rounded-[56px] h-[280px] flex flex-col items-center justify-center overflow-hidden shadow-2xl">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="flex items-baseline gap-3">
                <span className={cn(
                  'font-black tracking-tighter transition-all duration-300 tabular-nums text-[100px] leading-none',
            ...
            <div className="space-y-6">
              ...
              <div className="pt-6 border-t border-slate-800">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em] mb-1">VOLUMES CAPTURADOS</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <button onClick={handleCapture} disabled={isCapturing || !selectedProduct}
          ...
          <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center">
            <ArrowDownCircle className="w-6 h-6 text-primary" aria-hidden="true" />
          </div>
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Capturar Peso</span>
        </button>

        <button onClick={() => showToast('Scanner de Volume ativo. Bipe a etiqueta do volume.')}
        ...
        <button onClick={handleZero}
          className="h-28 bg-white dark:bg-slate-900 border-4 border-slate-100 dark:border-slate-800 rounded-[32px] flex flex-col items-center justify-center gap-2 hover:border-red-500 hover:bg-red-500/5 transition-all shadow-lg active:scale-95">
        ...
        <button onClick={handleSave} disabled={!isValidated || isBlocked || isSaving}
          className={cn(
          ...
          <div className="w-12 h-12 rounded-2xl bg-black/10 flex items-center justify-center">
            {isBlocked ? <Lock className="w-6 h-6" /> : isSaving ? <RefreshCcw className="w-6 h-6 animate-spin" /> : <Save className="w-6 h-6" />}
          </div>
          <span className="text-[10px] font-black uppercase tracking-[0.2em]">
            {isBlocked ? 'BLOQUEADO' : isSaving ? 'Salvando...' : 'Salvar Pesagem'}
          </span>
        </button>
      </div>
