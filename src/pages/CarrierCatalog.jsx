...
const emptyForm = () => ({
  ...
  cidade: '', uf: '', status: 'Ativo', tipo: 'transportadora',
});
...
export default function CarrierCatalog() {
  ...
  const fetchItems = async () => {
    ...
    const { data } = await supabase.from('clientes').select('*').eq('tipo', 'transportadora').order('razao_social');
    setItems(data || []);
  ...
  const handleSave = async () => {
    ...
    if (isNew) {
      const { data: inserted, error } = await supabase.from('clientes').insert({ ...formData, tipo: 'transportadora' }).select().single();
      if (error) { showToast(`Erro ao salvar: ${error.message}`, 'error'); return; }
      await fetchItems();
      setIsNew(false);
      showToast('Transportadora cadastrada com sucesso!');
    } else if (selectedId) {
  ...
  const confirmarDelete = async () => {
    ...
    setSelectedId(null);
    setFormData(emptyForm());
    showToast('Transportadora excluída.', 'info');
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col animate-in fade-in duration-700">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 max-w-sm w-full shadow-2xl border border-red-200">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 mb-6">
              Deseja excluir a transportadora <strong>"{selected?.razao_social || '—'}"</strong>?
            </p>
      ...
      <div className="bg-white dark:bg-slate-900 border-b-2 border-slate-100 dark:border-slate-800 px-6 py-5 relative overflow-hidden shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">7.6 Cadastro e Segurança</p>
            <h1 className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">7.6 Catálogo de Transportadoras</h1>
            <p className="text-xs text-slate-400 font-medium mt-0.5">Fretes · Rotas · Veículos · Parceiros Logísticos</p>
          </div>
          <div className="ml-auto flex items-center gap-2">
            <div className="relative group">
              ...
              <HelpTip text="A Integração via API Omie será conectada aqui. No futuro, as transportadoras serão importadas automaticamente." position="bottom" />
            </div>
            ...
            {isNew && (
              <div className="w-full text-left px-4 py-4 border-b border-slate-100 bg-secondary/10 border-l-4 border-l-secondary flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] font-black font-mono text-secondary">NOVA TRANSPORTADORA</p>
                  <p className="text-xs font-bold text-slate-600 leading-snug mt-0.5">Preencha o formulário ao lado</p>
                </div>
              </div>
            )}
            {filtered.length === 0 && !isNew && (
              <p className="text-center text-[11px] text-slate-400 py-8 font-medium">Nenhuma transportadora cadastrada.</p>
            )}
            {filtered.map(c => (
          ...
          {showDetail ? (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">
                      {isNew ? 'Nova Transportadora' : (formData.razao_social || '—')}
                    </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="col-span-1 md:col-span-2 bg-secondary/5 border-2 border-dashed border-secondary/30 rounded-3xl p-8 text-center space-y-3">
                  ...
                  <p className="text-xs text-slate-500 font-medium max-w-md mx-auto">
                    A API de Transportadoras (Consultar, Listar e Upsert) será conectada neste módulo.
                    Isso permitirá que os dados logísticos venham diretamente do ERP sem necessidade de redigitação.
                  </p>
                  <div className="flex justify-center gap-2 mt-4">
                    <span className="text-[10px] bg-secondary/20 text-secondary px-3 py-1 rounded-full font-black uppercase">API de Transportadoras v1</span>
            ...
            <div className="h-full flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="font-bold text-slate-400">Selecione uma transportadora ou clique em Nova Transportadora</p>
              </div>
            </div>
          )}
        </div>
