...
const SuporteGuias = () => {
    ...
    const StatusBox = ({ linha, coluna }) => {
        ...
        const address = `SG_${linha}${coluna}`;
            ...
            <div
                ...
                className={`
                    ...
                    ${occupied
                        ...
                        : 'bg-violet-500/10 text-violet-400 hover:scale-110 hover:z-10 cursor-pointer'
                    }
    ...
    return (
        <EnterprisePageBase
            title="2.21 Suporte Guias"
            subtitle="Monitoramento visual da área de suporte de guias"
            actions={
                <div className="flex items-center gap-3">
                    <div className="relative group">
                        <input
                            type="text"
                            placeholder="Buscar SKU ou Endereço (ex: SG_1A)..."
                            value={searchTerm}
                    ...
                    <div className="lg:col-span-3 bg-[#0a0a0a] rounded-2xl border border-white/5 p-6 flex flex-wrap gap-8 items-center justify-between">
                        <div className="flex gap-8">
                            <div className="flex flex-col gap-1">
                                ...
                                <span className="text-3xl font-black text-violet-500">0%</span>
                            </div>
                            <div className="flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-3 px-4 py-2 rounded-xl border bg-violet-500/10 border-violet-500/20">
                            <div className="w-2 h-2 rounded-full bg-violet-500 animate-pulse" />
                            <div className="flex flex-col">
                                <span className="text-[10px] font-black uppercase text-violet-400">Suporte Guias</span>
                                <span className="text-[8px] font-bold text-white/40">Área de Guias e Trilhos</span>
                            </div>
                        </div>
                    </div>
                    <div className="bg-violet-600/5 border border-violet-600/20 rounded-2xl p-6 flex flex-col justify-between">
                        <div className="flex items-center justify-between mb-4">
                            <Box className="text-violet-500" size={20} />
                            <span className="text-[9px] font-black bg-violet-600 text-white px-2 py-0.5 rounded uppercase">Info</span>
                        </div>
                        <h4 className="text-xs font-black text-white uppercase tracking-tight mb-1">Suporte Guias</h4>
                        <p className="text-[10px] text-white/50 leading-tight">Armazenagem de guias, trilhos e perfis estruturais. Prefixo SG_.</p>
                    </div>
                </div>
                        ...
                        <div className="fixed z-[100] bg-black/90 backdrop-blur-xl border border-white/20 p-4 rounded-xl shadow-2xl pointer-events-none animate-in fade-in zoom-in duration-200" style={{ left: '20px', bottom: '20px' }}>
                            <div className="flex flex-col gap-2 min-w-[200px]">
                                <div className="flex justify-between items-center border-b border-white/10 pb-2">
                                    <span className="text-[10px] font-black text-violet-400 tracking-widest">{hoveredPos.address}</span>
                                ...
                                {hoveredPos.data?.ocupado ? (
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-[10px] font-mono text-violet-400/80">{hoveredPos.data.sku}</span>
                                    </div>
                                ) : (
                    ...
                    <div className="flex flex-col gap-6 overflow-x-auto pb-10 custom-scrollbar">
                        <div className="flex items-center justify-between border-b border-white/5 pb-4">
                            <div className="flex items-center gap-4">
                                <div className="w-16 h-16 rounded-2xl bg-violet-500 flex items-center justify-center font-black text-2xl text-white shadow-xl shadow-violet-500/20">
                                    SG
                                </div>
                                <div>
                                    <h2 className="text-3xl font-black text-white uppercase tracking-tighter">Suporte Guias</h2>
                                    <p className="text-sm font-bold text-violet-400 uppercase tracking-widest flex items-center gap-2">
                                        Guias e Trilhos <ArrowRight size={14} className="text-white/20" /> {ROWS.length} linhas × {COLS.length} colunas <ArrowRight size={14} className="text-white/20" /> {TOTAL} posições
                                    </p>
                                </div>
                            </div>
