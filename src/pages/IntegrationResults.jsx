...
// Mapeamento semântico de status → classes Tailwind padrão
const STATUS_STYLE = {
    Sucesso: 'bg-green-500/10 text-green-600 border-green-500/20',
...
export default function IntegrationResults() {
    ...
    const [reintegrateDone, setReintegrateDone]  = useState(null); // id do último reintegrado com sucesso
    const timeoutRef = useRef(null);

    // Cleanup do timeout ao desmontar para evitar memory leak
    useEffect(() => {
        return () => {
    ...
    // Fechar modal de payload com Escape
    useEffect(() => {
        if (!showPayload) return;
        const onKey = (e) => { if (e.key === 'Escape') setShowPayload(null); };
        document.addEventListener('keydown', onKey);
        return () => document.removeEventListener('keydown', onKey);
    }, [showPayload]);

    // Fechar modal de período com Escape
    useEffect(() => {
        if (!showPeriodModal) return;
        const onKey = (e) => { if (e.key === 'Escape') setShowPeriodModal(false); };
        document.addEventListener('keydown', onKey);
        return () => document.removeEventListener('keydown', onKey);
    }, [showPeriodModal]);

    const handleReintegrate = (id) => {
        if (reintegratingId) return; // impede múltiplas reintegrações simultâneas
        setReintegratingId(id);
        if (timeoutRef.current) clearTimeout(timeoutRef.current);
        // ⚠️ INTEGRAÇÃO NECESSÁRIA: POST /api/webhooks/${id}/reintegrate
        timeoutRef.current = setTimeout(() => {
            setReintegratingId(null);
            setReintegrateDone(id);
            setShowPayload(null); // fecha modal após reintegração
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black tracking-tight flex items-center gap-3">
                        <Activity className="w-8 h-8 text-secondary" aria-hidden="true" /> 8.5 Resultados de Integração
                    </h1>
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        <Calendar className="w-4 h-4" aria-hidden="true" /> Período
                    </button>
                    <div className="bg-primary px-6 py-2 rounded-2xl border border-secondary/20 shadow-lg flex flex-col items-end">
                        <span className="text-[10px] font-black text-secondary/50 uppercase">Erros Hoje</span>
                        <span className="text-xl font-black text-secondary leading-none">02</span>
                    </div>
                </div>
            </div>

            {/* Feedback de reintegração bem-sucedida */}
            {reintegrateDone && (
                <div
                    role="status"
                    aria-live="polite"
                    className="flex items-center gap-3 px-5 py-3 bg-green-500/10 border border-green-500/20 rounded-2xl text-green-600 text-xs font-black animate-in fade-in duration-300"
                >
                    <CheckCircle2 className="w-4 h-4 shrink-0" aria-hidden="true" />
                    Registro <strong>{reintegrateDone}</strong> reintegrado com sucesso. Payload reenviado ao endpoint do cliente.
                    <button onClick={() => setReintegrateDone(null)} aria-label="Fechar aviso" className="ml-auto text-green-600/60 hover:text-green-600 transition-colors">
            ...
            <div className="bg-primary p-6 rounded-[2rem] shadow-xl border-b-8 border-secondary/20 relative overflow-hidden group">
                <div className="flex flex-wrap items-center justify-between gap-6 relative z-10">
                    <div className="flex bg-white/5 p-1.5 rounded-2xl border border-white/10" role="group" aria-label="Filtrar por status">
                    ...
                    <div className="flex-1 flex justify-end">
                        {/* ⚠️ INTEGRAÇÃO NECESSÁRIA: GET /api/webhooks/refresh */}
                        <button
                            aria-label="Atualizar dashboard de resultados"
                        ...
                        >
                            <RefreshCw className="w-4 h-4" aria-hidden="true" /> Atualizar Dashboard
                        </button>
                    </div>
                </div>
                {/* Background Decor */}
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 text-[10px] font-black uppercase tracking-widest text-slate-400">
                                <th scope="col" className="px-8 py-5">Data e Hora</th>
                                <th scope="col" className="px-8 py-5 text-center">Status</th>
                                <th scope="col" className="px-8 py-5">Interface / Destino</th>
                                <th scope="col" className="px-8 py-5">Identificador</th>
                                <th scope="col" className="px-8 py-5">Descrição do Log (Terceiro)</th>
                                <th scope="col" className="px-8 py-5 text-right">Ação</th>
                            </tr>
                        </thead>
                            ...
                            {filteredData.map(item => (
                                <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-all group">
                                    ...
                                    <td className="px-8 py-5 text-center">
                                        <span className={`px-2.5 py-1 text-[8px] font-black uppercase rounded-lg border inline-block min-w-[80px] ${STATUS_STYLE[item.status] ?? 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                                            {item.status}
                                        </span>
                                    </td>
                                    <td className="px-8 py-5">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-900 flex items-center justify-center">
                                                <ArrowRightLeft className="w-4 h-4 text-slate-400 group-hover:text-primary transition-colors" aria-hidden="true" />
                                            </div>
                                    ...
                                    <td className="px-8 py-5">
                                        {/* title exibe texto completo quando truncado */}
                                        <p
                                            title={item.description}
                                            className={`text-xs font-bold truncate max-w-[250px] ${item.status === 'Erro' ? 'text-red-500' : 'text-slate-500 italic'}`}
                                        >
                                            {item.description}
                                        </p>
                                    </td>
                                    <td className="px-8 py-5 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <button
                                                ...
                                                aria-label={`Ver payload JSON do webhook ${item.id}`}
                                            ...
                                            {item.status === 'Erro' && (
                                                <button
                                                    ...
                                                    aria-label={`Reintegrar log ${item.id}`}
                                                    title="Reintegrar Log"
                                                    disabled={reintegratingId !== null}
                                                    className="p-2 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                                >
                                                    <RefreshCw className={`w-4 h-4 ${reintegratingId === item.id ? 'animate-spin' : ''}`} aria-hidden="true" />
                                                </button>
                                            )}
                                        </div>
                                    </td>
                    ...
                    <div className="bg-slate-900 w-full max-w-2xl h-[70vh] rounded-[3.5rem] shadow-2xl flex flex-col overflow-hidden border border-white/10">
                        <div className="p-8 border-b border-white/10 flex items-center justify-between bg-white/[0.02]">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 id="payload-modal-title" className="text-xl font-black text-white italic">Payload da Transação</h3>
                            ...
                            <button
                                ...
                                aria-label="Fechar modal de payload"
                                className="p-2 bg-white/5 hover:bg-red-500/20 hover:text-red-400 rounded-full transition-all text-white"
                            >
                        ...
                        <div className="p-8 border-t border-white/10 bg-white/[0.02]">
                            {/* Botão unificado com reintegratingId state para evitar dupla reintegração */}
                            <button
                                onClick={() => handleReintegrate(showPayload.id)}
                                disabled={reintegratingId !== null}
                                aria-label={`Forçar reenvio do webhook ${showPayload.id}`}
                                className="w-full py-5 bg-secondary text-primary rounded-[2rem] font-black text-[10px] tracking-widest uppercase hover:bg-secondary/90 transition-all shadow-xl shadow-secondary/10 flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {reintegratingId === showPayload.id ? (
                                    <RefreshCw className="w-4 h-4 animate-spin" aria-hidden="true" />
                                ) : (
                                    <Play className="w-4 h-4 fill-current" aria-hidden="true" />
                                )}
                                {reintegratingId === showPayload.id ? 'Reintegrando...' : 'Forçar Reenvio do Webhook'}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                >
                    <div className="bg-white dark:bg-slate-800 w-full max-w-sm p-8 rounded-[2.5rem] shadow-2xl space-y-6">
                        <div className="flex items-center justify-between">
                            <h3 id="period-modal-title" className="text-xl font-black italic text-primary">Filtrar Período</h3>
                            <button
                        ...
                        <div className="space-y-4">
                            <div className="space-y-1 text-left">
                                <label htmlFor="date-start" className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">
                                    Data Início
                                </label>
                                <input
                                    id="date-start"
                            ...
                            <div className="space-y-1 text-left">
                                <label htmlFor="date-end" className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">
                                    Data Fim
                                </label>
                                <input
                                    id="date-end"
                        ...
                        {/* ⚠️ INTEGRAÇÃO NECESSÁRIA: filtro de data real contra API */}
                        <button
                            onClick={() => setShowPeriodModal(false)}
                            className="w-full py-4 bg-primary text-secondary rounded-2xl font-black text-[10px] tracking-widest uppercase hover:bg-primary/95 transition-all shadow-xl shadow-primary/20"
                        >
